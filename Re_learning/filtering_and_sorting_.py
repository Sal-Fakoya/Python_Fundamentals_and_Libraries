"""
                                        FILTERING: filter(pred, iterable)
--> Filtering is basically the selection of a subset of items based on whether a condition is true or not.
--> A predicate function is simply a function of one or more arguments that returns True or False.
--> For filtering, we need to have:
    --> An iterable
    --> A predicate function
    --> Only keep the items for which predicate functions evaluates to True.
--> It generates a new iterable that only contains the elements of the new iterable where the predicate function is True
--> Filter is a lazy iterator, meaning we can only iterate throught it once

                            PREDICATE FUNCTIONS CREATED VIS LAMBDA
--> We can also use a predicate function created via a lambda.
...
#
# closed= (value[3] for value in quotes)
# opened = (value[1] for value in quotes)

higher = (value for value in quotes if value[4] > value[1])
print(list(higher))

higher1 = filter(lambda rec: rec[4] > rec[1], quotes)
print(list(higher1))


# print(list(higher) == list(higher1))

# Writing Predicate function
...
ans = filter(closed_higher, quotes)

print(list(ans))

higher = (value for value in quotes if closed_higher(value))
print(list(higher))
